<template>
  <div id="payment-det">
    <div class="container">
      <div id="generate-pdf" ref="content">
        <div class="header d-flex">
          <img
            ...
            alt
          />
          <div class="title ml-auto">Invoice</div>
        </div>
        <div class="spacer-20"></div>
        <div class="row">
          <div class="item ml-auto">
            <div class="sub-title">Invoice No.</div>
            <div class="sub-item">001/2019</div>
            <div class="sub-title">Invoice No.</div>
            <div class="sub-item">001/2019</div>
          </div>
        </div>
        ...
        <div class="row d-flex">
          <table class="total ml-auto">
            <tr>
              <th>SUB TOTAL</th>
              <td>Rp {{subTotal}}</td>
            </tr>
            <tr>
              <th>BIAYA ADMINISTRASI (10%)</th>
      ...
      <div class="row d-flex">
        <button class="btn btn-primary ml-auto" @click.prevent="download">BAYAR</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    subTotal: function() {
      var total = 0;
      for (var i = 0; i < this.dataTransaksi.length; i++) {
        total = parseInt(this.dataTransaksi[1].jumlah) + total;
      }
      return total;
    },
    administrasi: function() {
      var total = (this.subTotal * 10) / 100;
      return total;
    },
    total: function() {
      var total = this.subTotal - this.administrasi;
      return total;
    }
  },
  methods: {
    download() {
      html2canvas(document.querySelector("#generate-pdf"), {
        imageTimeout: 1000,
        useCORS: true
      }).then(canvas => {
